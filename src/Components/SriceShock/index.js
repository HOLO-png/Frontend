...
import { Avatar, Badge, Card, Progress, Row } from "antd";
    ...
    if (products) {
      return products
        ? products.filter((item) => {
            return handleChangeProductPrice(item.priceOld, item.price) > 10;
          })
        : "";
    }
  };
        ...
        >
          <Slider {...settings}>
            {handleRenderProductUI(mobile_api)
              ? handleRenderProductUI(mobile_api).map((item, index) => (
                  <div style={{ width: 247 }} key={index}>
                    <Link
                      to={`/product/${item.category}/${item.name.replace(
                        /[^\w\s]/gi,
                        ""
                      )}/${item._id}`}
                    >
